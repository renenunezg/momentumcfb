...
def _validate_identity(identifier: int, name: str, label: str) -> None:
    if identifier <= 0:
        raise ValueError(f"{label}_id must be positive")
    if not name.strip():
...
class TeamRating:
    """Weekly team strength on a neutral field against an average FBS team.
    ...
    def __post_init__(self) -> None:
        _validate_identity(self.team_id, self.team, "team")
        ...
        if self.expected_possessions <= 0:
            raise ValueError("expected_possessions must be positive")
