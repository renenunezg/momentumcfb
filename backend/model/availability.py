...
def apply_qb_availability(
    ...
    for side in ("home", "away"):
        ...
        flagged[side] = out[column].fillna(False).astype(bool) if column in out else pd.Series(False, index=out.index)
    ...
    if not (fresh["home"].any() or fresh["away"].any()):
        return out
    out["expected_home_points"] = (out["expected_home_points"] - home_loss).clip(lower=0.0)
    out["expected_away_points"] = (out["expected_away_points"] - away_loss).clip(lower=0.0)
    ...
    if "market_informed_home_margin" in out:
        ...
        out["market_informed_home_margin"] = out["market_informed_home_margin"] + share * net
