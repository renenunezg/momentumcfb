...
-- cfb-picks-v5 prices sides and totals with the empirical dispersion around
-- the market-informed line instead of the pure model's wider predictive
-- spread, and gates on points of edge beyond the price's break-even line, the
-- same yardstick for favourites and underdogs. margin_sd and total_sd now
-- record the dispersion the pick was priced with. A withdrawn pick is settled
-- void with an explicit settlement_reason instead of being deleted.
alter table cfb.recommendations add column edge_points double precision;
alter table cfb.recommendations add column settlement_reason text;
alter table cfb.recommendations drop constraint recommendation_eligibility_v4;
alter table cfb.recommendations add constraint recommendation_eligibility_v5
check ((status = 'no_play' and stake_units = 0) or
    (status = 'recommended' and stake_units = 1 and
     ((home_missing_input_count = 0 and away_missing_input_count = 0) or
      (policy_version in ('cfb-picks-v2', 'cfb-picks-v3', 'cfb-picks-v4', 'cfb-picks-v5')
     ...
     ((policy_version in ('cfb-picks-v2', 'cfb-picks-v3', 'cfb-picks-v4')
      ...
      (policy_version = 'cfb-picks-v5'
       and edge_points >= 2 and edge_points < 'Infinity'::float8)) and
...
begin
  ...
  if TG_OP = 'UPDATE' then
    ...
    if old.status = 'recommended' or old.start_date <= clock_timestamp() then
      if (to_jsonb(new) - array['outcome','home_points','away_points','profit_units','graded_at','settlement_reason'])
        is distinct from
         (to_jsonb(old) - array['outcome','home_points','away_points','profit_units','graded_at','settlement_reason']) then
