...
-- cfb-picks-v6 prices moneylines from the consensus market margin moved 0.2
-- toward the pure model (the 0.5 blend priced moneyline picks 8 to 9 points
-- above their realised win rate on 2021 through 2025) and promotes the
...
-- edge_points between 0 and 2, so the eligibility gate for v6 requires
-- positive edge and positive EV; v5 rows keep the 2-point gate.
alter table cfb.recommendations drop constraint recommendation_eligibility_v5;
alter table cfb.recommendations add constraint recommendation_eligibility_v6
check ((status = 'no_play' and stake_units = 0) or
    (status = 'recommended' and stake_units = 1 and
     ((home_missing_input_count = 0 and away_missing_input_count = 0) or
      (policy_version in ('cfb-picks-v2', 'cfb-picks-v3', 'cfb-picks-v4', 'cfb-picks-v5', 'cfb-picks-v6')
