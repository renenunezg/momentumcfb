...
-- A published pick that a newer model version no longer makes is withdrawn
-- before kickoff: settled void with settlement_reason 'model_withdrawn' and
-- the original line, price and decision kept as the record. Only a model
-- version change withdraws; line movement alone never does. The trigger
-- admits the new reason under the same pre-kickoff rule as a policy
-- withdrawal.
...
begin
  ...
  if new.outcome = 'pending' then
    ...
    if new.outcome = 'void' then
      ...
      if new.settlement_reason in ('policy_withdrawn', 'model_withdrawn')
